...
describe('Autenticação', () => {
  ...
  describe('POST /api/v1/auth/register', () => {
    it('cria conta e retorna access + refresh token', async () => {
      ...
      const res = await api.post('/api/v1/auth/register').send({ email, password: 'senha123456', name: 'Fulana' });
    ...
    it('rejeita e-mail duplicado', async () => {
      const email = uniqueEmail();
      await api.post('/api/v1/auth/register').send({ email, password: 'senha123456', name: 'Fulana' });
      const res = await api.post('/api/v1/auth/register').send({ email, password: 'outrasenha1', name: 'Fulana' });

      expect(res.status).toBe(409);
    });

    it('rejeita senha curta', async () => {
      const res = await api.post('/api/v1/auth/register').send({ email: uniqueEmail(), password: '123', name: 'Fulana' });
  ...
  describe('POST /api/v1/auth/login', () => {
    it('autentica com credenciais corretas', async () => {
      ...
      const res = await api.post('/api/v1/auth/login').send({ email: user.email, password: 'senha123456' });
    ...
    it('rejeita senha errada sem dizer se o e-mail existe', async () => {
      const user = await registerUser();
      const res = await api.post('/api/v1/auth/login').send({ email: user.email, password: 'senhaErrada' });
      expect(res.status).toBe(401);
    });

    it('rejeita e-mail inexistente com a mesma mensagem genérica', async () => {
      const res = await api.post('/api/v1/auth/login').send({ email: uniqueEmail(), password: 'qualquer1' });
      expect(res.status).toBe(401);
  ...
  describe('GET /api/v1/auth/me', () => {
    it('retorna o perfil do usuário autenticado', async () => {
      const user = await registerUser();
      const res = await api.get('/api/v1/auth/me').set('Authorization', `Bearer ${user.accessToken}`);
      expect(res.status).toBe(200);
    ...
    it('rejeita token malformado', async () => {
      const res = await api.get('/api/v1/auth/me').set('Authorization', 'Bearer token-invalido');
      expect(res.status).toBe(401);
    });
  });
  ...
  describe('POST /api/v1/auth/refresh', () => {
    ...
    it('bloqueia o reuso do refresh token depois de rotacionado', async () => {
      ...
      const reused = await api.post('/api/v1/auth/refresh').send({ refreshToken: user.refreshToken });
      expect(reused.status).toBe(401);
    });

    it('rejeita refresh token inventado', async () => {
      const res = await api.post('/api/v1/auth/refresh').send({ refreshToken: 'a'.repeat(64) });
      expect(res.status).toBe(401);
    });
  });

  describe('POST /api/v1/auth/logout + GET /api/v1/auth/sessions', () => {
    it('lista a sessão criada no registro', async () => {
      const user = await registerUser();
      const res = await api.get('/api/v1/auth/sessions').set('Authorization', `Bearer ${user.accessToken}`);
      expect(res.status).toBe(200);
    ...
    it('revoga a sessão — refresh deixa de funcionar depois do logout', async () => {
      ...
      const refresh = await api.post('/api/v1/auth/refresh').send({ refreshToken: user.refreshToken });
  ...
  describe('Recuperação de senha', () => {
    it('fluxo completo: pedir reset, trocar senha, logar com a nova e não com a antiga', async () => {
      ...
      const reset = await api.post('/api/v1/auth/reset-password').send({ token: rawToken, newPassword });
      expect(reset.status).toBe(200);

      // Token de reset é de uso único.
      const reused = await api.post('/api/v1/auth/reset-password').send({ token: rawToken, newPassword: 'outraSenha3' });
      expect(reused.status).toBe(400);

      const oldLogin = await api.post('/api/v1/auth/login').send({ email: user.email, password: oldPassword });
      expect(oldLogin.status).toBe(401);

      const newLogin = await api.post('/api/v1/auth/login').send({ email: user.email, password: newPassword });
    ...
    it('trocar a senha revoga as sessões existentes (logout em todos os aparelhos)', async () => {
      ...
      const rawToken = sendPasswordResetEmail.mock.calls[0][1] as string;
      await api.post('/api/v1/auth/reset-password').send({ token: rawToken, newPassword: 'senhaNova2' });

      const refreshAfterReset = await api.post('/api/v1/auth/refresh').send({ refreshToken: user.refreshToken });
      expect(refreshAfterReset.status).toBe(401);
    });

    it('rejeita token de reset inventado', async () => {
      const wrongReset = await api.post('/api/v1/auth/reset-password').send({ token: 'token-que-nao-existe', newPassword: 'novaSenha2' });
    ...
    it('responde com a mesma mensagem para e-mail existente e inexistente (sem enumeração)', async () => {
      const user = await registerUser();
      const existing = await api.post('/api/v1/auth/forgot-password').send({ email: user.email });
      const missing = await api.post('/api/v1/auth/forgot-password').send({ email: uniqueEmail() });
  ...
  describe('DELETE /api/v1/auth/me (LGPD)', () => {
    ...
    it('exclui a conta e invalida a sessão', async () => {
      ...
      const del = await api.delete('/api/v1/auth/me').set('Authorization', `Bearer ${user.accessToken}`).send({ password });
