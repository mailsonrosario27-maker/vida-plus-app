...
describe('Gamificação', () => {
  ...
  it('não desbloqueia nenhuma conquista pra usuário recém-criado sem atividade', async () => {
    ...
    const res = await api.get('/api/v1/achievements').set('Authorization', `Bearer ${user.accessToken}`);
  ...
  it('desbloqueia "primeiro dia" e soma pontos ao registrar água', async () => {
    ...
    const log = await api.post('/api/v1/water').set('Authorization', `Bearer ${user.accessToken}`).send({ amountMl: 250 });
    expect(log.status).toBe(201);
    expect(log.body.unlockedAchievements).toContain('Primeiro dia');

    const achievements = await api.get('/api/v1/achievements').set('Authorization', `Bearer ${user.accessToken}`);
  ...
  it('não desbloqueia a mesma conquista duas vezes', async () => {
    ...
    const first = await api.post('/api/v1/water').set('Authorization', `Bearer ${user.accessToken}`).send({ amountMl: 250 });
    expect(first.body.unlockedAchievements).toContain('Primeiro dia');

    const second = await api.post('/api/v1/water').set('Authorization', `Bearer ${user.accessToken}`).send({ amountMl: 250 });
  ...
  it('meta de água batida desbloqueia a conquista correspondente', async () => {
    ...
    const res = await api.post('/api/v1/water').set('Authorization', `Bearer ${user.accessToken}`).send({ amountMl: 500 });
    expect(res.body.unlockedAchievements).toContain('Meta de água');
  });

  it('/api/v1/progress/summary calcula o percentual geral do dia a partir dos registros reais', async () => {
    const user = await registerUser();
    await prisma.profile.update({ where: { userId: user.id }, data: { waterGoalMl: 1000 } });
    await api.post('/api/v1/water').set('Authorization', `Bearer ${user.accessToken}`).send({ amountMl: 1000 });

    const summary = await api.get('/api/v1/progress/summary').set('Authorization', `Bearer ${user.accessToken}`);
