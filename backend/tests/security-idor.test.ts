...
describe('Segurança — IDOR (jejum e treino)', () => {
  ...
  describe('Jejum', () => {
    it('usuário B não consegue pausar o jejum do usuário A', async () => {
      ...
      const start = await api.post('/api/fasting/start').set('Authorization', `Bearer ${a.accessToken}`).send({});
      const sessionId = start.body.id;

      const attack = await api.post(`/api/fasting/${sessionId}/pause`).set('Authorization', `Bearer ${b.accessToken}`);
      expect(attack.status).toBe(404);

      // Confirma que o estado real não mudou — não é só o response code.
      const active = await api.get('/api/fasting/active').set('Authorization', `Bearer ${a.accessToken}`);
    ...
    it('usuário B não consegue encerrar o jejum do usuário A nem ganhar os pontos dele', async () => {
      ...
      const attack = await api.post(`/api/fasting/${sessionId}/end`).set('Authorization', `Bearer ${b.accessToken}`);
      expect(attack.status).toBe(404);

      const bAchievements = await api.get('/api/achievements').set('Authorization', `Bearer ${b.accessToken}`);
    ...
    it('usuário B não consegue cancelar o jejum do usuário A', async () => {
      ...
      const attack = await api.post(`/api/fasting/${start.body.id}/cancel`).set('Authorization', `Bearer ${b.accessToken}`);
      expect(attack.status).toBe(404);
    });
    ...
    it('chamar /end duas vezes na mesma sessão não reaplica pontos (farming)', async () => {
      ...
      const firstEnd = await api.post(`/api/fasting/${start.body.id}/end`).set('Authorization', `Bearer ${a.accessToken}`);
      expect(firstEnd.status).toBe(200);

      const afterFirst = await api.get('/api/achievements').set('Authorization', `Bearer ${a.accessToken}`);
      const pointsAfterFirst = afterFirst.body.totalPoints;
      expect(pointsAfterFirst).toBeGreaterThan(0);

      const secondEnd = await api.post(`/api/fasting/${start.body.id}/end`).set('Authorization', `Bearer ${a.accessToken}`);
      expect(secondEnd.status).toBe(404);

      const afterSecond = await api.get('/api/achievements').set('Authorization', `Bearer ${a.accessToken}`);
  ...
  describe('Treino', () => {
    it('usuário B não consegue completar a sessão de treino do usuário A nem ganhar os pontos dele', async () => {
      ...
      const start = await api.post(`/api/workouts/${workout.id}/start`).set('Authorization', `Bearer ${a.accessToken}`);
      const sessionId = start.body.id;
    ...
    it('usuário B não consegue cancelar a sessão de treino do usuário A', async () => {
      ...
      const attack = await api
        .post(`/api/workouts/sessions/${start.body.id}/cancel`)
        .set('Authorization', `Bearer ${b.accessToken}`);
      expect(attack.status).toBe(404);
    });
    ...
    it('completar a mesma sessão duas vezes não reaplica pontos (farming)', async () => {
      ...
      const firstComplete = await api
        .post(`/api/workouts/sessions/${start.body.id}/complete`)
  ...
  describe('Água (regressão de escopo por dono, caso mais simples)', () => {
    it('usuário B não consegue apagar um registro de água do usuário A', async () => {
      ...
      const log = await api.post('/api/water').set('Authorization', `Bearer ${a.accessToken}`).send({ amountMl: 250 });
      expect(log.status).toBe(201);

      const attack = await api.delete(`/api/water/${log.body.log.id}`).set('Authorization', `Bearer ${b.accessToken}`);
      expect(attack.status).toBe(204); // deleteMany com where errado não acha nada, não é erro — mas...

      const todayA = await api.get('/api/water/today').set('Authorization', `Bearer ${a.accessToken}`);
